...
describe('checkDispatchDeclaration', () => {
  ...
  it('warns when a template dispatches a task type the rule does not declare', async () => {
    const report = await harness.audit(
      buildAgent({
        providers: {
          webhook: [{ name: 'triage', messageTemplate: 'templates/triage.md', tools: [] }],
          internal: [handleCancellation],
        },
        templates: {
          'triage.md': 'curl /api/tasks -d \'{ "taskType": "handle-cancellation" }\'\n',
          'handle.md': 'noop',
        },
      }),
    );
