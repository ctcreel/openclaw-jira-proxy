...
describe('callback dedupe', () => {
  ...
  it('returns true on the first delivery of an event_id', async () => {
    const first = await recordCallbackEvent('job-1:testable', redis);
    expect(first).toBe(true);
  });

  it('returns false on subsequent deliveries of the same event_id', async () => {
    await recordCallbackEvent('job-1:testable', redis);
    const second = await recordCallbackEvent('job-1:testable', redis);
    expect(second).toBe(false);
  });

  it('distinguishes between event_ids that share a job_id', async () => {
    expect(await recordCallbackEvent('job-1:working', redis)).toBe(true);
    expect(await recordCallbackEvent('job-1:question_pending', redis)).toBe(true);
    expect(await recordCallbackEvent('job-1:working', redis)).toBe(false);
  });

  it('distinguishes between event_ids that share a state', async () => {
    expect(await recordCallbackEvent('job-1:testable', redis)).toBe(true);
    expect(await recordCallbackEvent('job-2:testable', redis)).toBe(true);
  });
