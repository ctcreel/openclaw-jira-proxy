...
- Add a privileged dispatch route (`POST /webhooks/system/builder`) authenticated by a new internal-bearer signature strategy, with a dedicated BullMQ queue and Builder runner registration. The dispatch payload carries `agentName`, which Builder's runner resolves against `AGENTS_CONFIG` to find the target repo, path, Builder bot identity for that repo, branch convention, operator allowlist, and `testableMechanism`.
...
- **clawndom**: new `src/system-agents/builder/`, route + controller for `POST /webhooks/system/builder`, internal-bearer strategy in `src/strategies/`, dedicated BullMQ queue and Builder runner registration, callback route + Redis-backed idempotency dedup, Zod schemas for dispatch / callback / resume / reply-context payloads, per-agent config extensions (Builder bot reference, branch convention, allowlist, `testableMechanism`, supervisor webhook URL).
- **Each opting-in agent**: new `dispatch_to_builder` tool definition added to the agent's tool registry, privileged-route template variant added, tool-grant config updated so the tool is loaded only on privileged routes, declared `testableMechanism` and operator allowlist.
