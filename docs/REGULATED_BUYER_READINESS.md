...
1. **Anthropic registration**: the `tool_use` schema sent to the API contains only `name`, `description`, and `input_schema`. The `requires`/`secrets` configuration is NEVER included. Inspection: `src/services/tools/mcp-bridge.ts → buildMCPRunFiles` — the materialized `tool-config.json` has the secret aliases (for the MCP server's own knowledge) but the value-bearing `CLAWNDOM_TOOL_CREDS` env is passed only to the spawned MCP server subprocess, never serialized into prompt content.
2. **Agent process environment**: the runner subprocess inherits `process.env` BUT no credential values are added to that env. Slack tokens, Jira tokens, Gmail service-account JSON paths — none of these are exported into the env Clawndom passes to `claude`. Verification: `src/services/tools/executor.ts → runSubprocess` passes credentials only via stdin JSON.
3. **Audit log**: per "Redaction" above, exact-match scrubbing on both `args` and `result_summary`.
...
- **Credential-leakage probe** (`tests/integration/credential-leakage-probe.test.ts`) — adversarial regression guard. An "evil" impl tries to exfiltrate the credential via every in-process path: `os.environ` direct lookup, full env dump, `/proc/self/environ`. The contract verified: the literal credential value MUST NOT be visible to the impl through any of these channels. (The MCP server `os.environ.pop`s `CLAWNDOM_TOOL_CREDS` at startup; redaction covers the audit log; `/proc/self/environ` reflects the scrubbed env on Linux. macOS-skip.)
